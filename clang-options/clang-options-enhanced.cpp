...
static inline size_t bitBytes() { return (flagList.size() + 7) / 8; }
static inline size_t requiredLen() { return 2 + bitBytes(); }
...
std::string generateTestFileName(const std::vector<uint8_t> &data) {
    ...
    // scale instead of modulo
    uint16_t fileCount = getEnvOrDefaultInt("FILE_COUNT", 1811 );
    uint16_t fileIndex = (static_cast<uint32_t>(rawValue) * static_cast<uint32_t>(fileCount)) >> 16;
...
std::string decodeFlagsFromBinarySub_Bitset(const std::vector<uint8_t> &data) {
    std::string flags;

    const size_t FLAG_BITS = flagList.size();
    const size_t BIT_BYTES = (FLAG_BITS + 7) / 8;

    for (size_t i = 0; i < FLAG_BITS; ++i) {
        size_t byteIdx = 2 + (i >> 3);
        if (byteIdx >= data.size()) break; // missing bits => default off
        uint8_t mask = 1u << (i & 7);
        if (data[byteIdx] & mask) {
            flags += flagList[i] + " ";
        }
    }
    return flags;
...
int main(int argc, char* argv[]) {
    ...
    if (!fileBin.empty()) {
        ...
        subflags = decodeFlagsFromBinarySub_Bitset(data);   // uses 2 bytes
