...
#ifndef GRAYC_BUILD_TOOL
#define GRAYC_BUILD_TOOL 0
#endif
...
void ConstantMutatorFrontendAction::EndSourceFileAction() {
  ...
  if (m_out) {
    ...
    m_rewriter->getEditBuffer(src_mngr.getMainFileID()).write(os);
    os.flush();
    *m_out = std::move(s);
    return;
  }
...
int main(int argc, const char **argv) {
  ...
  auto ExpParser = CommonOptionsParser::create(argc_new, argv, MyToolCategory);
  if (!ExpParser) {
