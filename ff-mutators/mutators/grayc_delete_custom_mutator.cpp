...
#include "DeleteMutator.h"                 // your GrayC pass
...
static bool run_delete_on_string(const std::string &src, std::string &out, unsigned long seed) {
  GrayCCustomRandom::CreateInstance(seed, 6);           // matches your Delete mutator dice :contentReference[oaicite:6]{index=6}
  ...
  class Factory : public FrontendActionFactory {
    std::string &Out;
  public: explicit Factory(std::string &o) : Out(o) {}
    std::unique_ptr<FrontendAction> create() override {
      return std::make_unique<DeleteMutatorFrontendAction>(&Out); // uses the “string sink” we added
    }
  } factory(out);
...
extern "C" {
  ...
  const char *afl_custom_describe(void *, size_t) { return "grayc-delete"; }
  size_t afl_custom_fuzz(void *data,
    ...
    if (!sp.ok) return 0;
    std::string mutated;
    if (!run_delete_on_string(sp.program, mutated, (unsigned long)per_call_seed)) return 0;
