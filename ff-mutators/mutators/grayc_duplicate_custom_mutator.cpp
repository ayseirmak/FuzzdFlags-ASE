...
namespace {
class DuplicateCaptureAction : public ASTFrontendAction {
  Rewriter RW;
  std::string *Out = nullptr;
public:
  explicit DuplicateCaptureAction(std::string *o) : Out(o) {}
  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef) override {
    RW.setSourceMgr(CI.getSourceManager(), CI.getLangOpts());
    // Reuse your ASTConsumer
    return std::make_unique<DuplicateMutatorASTConsumer>(&CI, &RW);
  }
...
static bool run_duplicate_on_string(const std::string &src,
                                    std::string &out,
                                    unsigned long seed) {
  GrayCCustomRandom::CreateInstance(seed, 2);  // matches original DuplicateMutator main()
  ...
  public:
    ...
    std::unique_ptr<FrontendAction> create() override {
      return std::make_unique<DuplicateCaptureAction>(&Out);
    }
  } factory(out);
...
extern "C" {
  ...
  size_t afl_custom_fuzz(void *data,
    ...
    if (!sp.ok) return 0;
    std::string mutated;
    if (!run_duplicate_on_string(sp.program, mutated, (unsigned long)per_call_seed)) return 0;
