...
static bool run_const_on_string(const std::string &src, std::string &out, unsigned long seed) {
  GrayCCustomRandom::CreateInstance(seed, 65000);        // matches your Constant mutator dice :contentReference[oaicite:8]{index=8}
  ...
  public: explicit Factory(std::string &o) : Out(o) {}
    std::unique_ptr<FrontendAction> create() override {
      return std::make_unique<ConstantMutatorFrontendAction>(&Out);
    }
  } factory(out);
...
extern "C" {
  ...
  size_t afl_custom_fuzz(void *data,
    ...
    if (!sp.ok) return 0;
    std::string mutated;
    if (!run_const_on_string(sp.program, mutated, (unsigned long)per_call_seed)) return 0;
