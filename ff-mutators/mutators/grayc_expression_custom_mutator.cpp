...
#include "ExpressionMutator.h"               // ensure FrontendAction can sink to std::string*
...
static bool run_expr_on_string(const std::string &src,
                               std::string &out,
                               unsigned long seed) {
  GrayCCustomRandom::CreateInstance(seed, 60);
  ...
  public:
    ...
    std::unique_ptr<FrontendAction> create() override {
      return std::make_unique<ExpressionMutatorFrontendAction>(&Out);
    }
  } factory(out);
...
extern "C" {
  ...
  struct FFState {
    afl_state_t *afl;   // available if later you want to pull AFL-side randomness
    uint64_t     base;  // base seed from afl_custom_init()
    uint64_t     entry; // hash of current queue filename (set in afl_custom_queue_get)
    uint64_t     iter;  // per-entry call counter (incremented in afl_custom_fuzz)
  };
  ...
  size_t afl_custom_fuzz(void *data,
    ...
    if (!sp.ok) return 0;
    std::string mutated;
    if (!run_expr_on_string(sp.program, mutated, (unsigned long)per_call_seed)) return 0;
