...
CLANG_19="/opt/llvm-19/bin/clang-19"                     
...
do_compile_and_run() {
  ...
  local outbase_raw="$5"
  local outbase=$(echo "$outbase_raw" | sed 's|[^A-Za-z0-9._-]|_|g')
  ...
  (timeout "${RUN_TIMEOUT}s" "$bin_out" 10000000) >> "$run_log" 2>&1
  run_rc=$?
...
for f in "$FUZZED_DIR"/*; do
  ...
  checker_out=$($CLANG_OPTIONS_BIN --filebin "$f" --checker 2>/dev/null || true)
  ...
  mutated_flags="$local_flags"

  out_base=$(echo "case_$(basename "$f")" | sed 's|[^A-Za-z0-9._-]|_|g')
  ...
  for cc in "gcc-14" "clang-19" "$target_name"; do
  ...
  # For each compiler => compile & run
    ...
    do_compile_and_run "$CLANG_19" "clang-19" "$local_source" "$mutated_flags" "$out_base"
  ...
  for order in "gcc-14" "clang-19" "$target_name"; do
